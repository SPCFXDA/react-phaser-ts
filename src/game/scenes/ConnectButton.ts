...
export class ConnectButton extends GameObjects.Container {
    ...
    constructor(scene: Scene, x: number, y: number, text: string, clickHandler: () => void) {
        ...
        this.normalImage = scene.add.image(0, 0, 'buttonRoundUP');
        this.pressedImage = scene.add.image(0, 0, 'buttonRoundDOWN');
        this.disabledImage = scene.add.image(0, 0, 'buttonRoundDOWN');
        this.loadingImage = scene.add.image(0, 0, 'buttonRoundDOWN');
